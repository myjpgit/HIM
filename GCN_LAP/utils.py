...
def preprocess_adj(adj):
    ...
    adj_normalized = normalize_adj(adj+ sp.eye(adj.shape[0])) #拉普拉斯矩阵设置参数
...
def extract_adj(data):
    ...
    if data=='advisor_advisee':
        ...
        aut_laplacian = sio.loadmat('../' + data + '/refined_data/node_laplacian.mat')
        aut_laplacian_1 = aut_laplacian['node_laplacian_1']
        aut_laplacian_2 = aut_laplacian['node_laplacian_2']
        author_institution = sio.loadmat('../' + data + '/refined_data/end_aut_institution.mat')
        aut_ins = author_institution['author_institution']
        author_paper = sio.loadmat('../' + data + '/refined_data/end_author_paper.mat')
        aut_pap = author_paper['author_paper']
        adj = np.load('../' + data + '/refined_data/adj.npy')
        efeature = np.load('../' + data + '/refined_data/efeature.npy')
        return aut_aut_1, aut_aut_2, aut_laplacian_1, aut_laplacian_2, aut_ins, aut_pap, np.array(adj), np.array(efeature)
    elif data == 'enron':
        ...
        node_laplacian = sio.loadmat('../' + data + '/refined_data/node_laplacian.mat')
        node_degree = sio.loadmat('../' + data + '/refined_data/node_node_degree.mat')
        adj=np.load('../'+data+'/refined_data/adj.npy')
        efeature=np.load('../'+data+'/refined_data/efeature.npy')
        return node_node['node_node_1'],node_node['node_node_2'],node_laplacian['node_laplacian_1'],node_laplacian['node_laplacian_2'],node_feature['node_feature'],np.array(adj),np.array(efeature)
    elif data=='same_city':
        node_node=sio.loadmat('../'+data+'/refined_data/node_node.mat')
        node_feature = sio.loadmat('../' + data + '/refined_data/end_node_embedding.mat')
        node_laplacian=sio.loadmat('../'+data+'/refined_data/node_laplacian.mat')
        adj=np.load('../'+data+'/refined_data/adj.npy')
        efeature=np.load('../'+data+'/refined_data/efeature.npy')
        return node_node['node_node_1'], node_node['node_node_2'], node_laplacian['node_laplacian_1'], node_laplacian[
            'node_laplacian_2'], node_feature['node_feature'], np.array(adj), np.array(efeature)
...
def extract_val_data(data):
    row_edge_idx=[]
    col_pos_edge_idx=[]
    with open('../{}/refined_data/val_data.txt'.format(data)) as file:
        for line in file:
