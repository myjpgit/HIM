...
def gen_het_rand_walk():
    a_a_list_train = [[] for k in range(10195)]
    ...
    for i in range(10):    #for i in range(self.args.walk_n):
        for j in range(10195):
            if len(a_a_list_train[j]):
                curNode = "a" + str(j)
                het_walk_f.write(curNode + " ")
                for l in range(30 - 1):    #for l in range(self.args.walk_L - 1):
                    if curNode[0] == "a":
