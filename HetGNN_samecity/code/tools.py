...
class HetAgg(nn.Module):
    def __init__(self, args, feature_list, a_neigh_list_train, a_train_id_list):
        ...
        self.a_content_rnn = nn.LSTM(embed_d, embed_d, 1, bidirectional = False)

        self.a_neigh_rnn = nn.LSTM(embed_d, embed_d, 1, bidirectional = False)
    ...
    def a_content_agg(self, id_batch): #heterogeneous content aggregation
        ...
        a_net_embed_batch = self.feature_list[0][id_batch]
    ...
    def node_neigh_agg(self, id_batch, node_type=1): #type based neighbor aggregation with rnn
        embed_d = self.embed_d
        ...
        if node_type == 1:
            print(self.a_content_agg(id_batch).shape)
    ...
    def node_het_agg(self, id_batch, node_type=1): #heterogeneous neighbor aggregation
        ...
        #compute weights
        concate_embed = torch.cat((c_agg_batch_2, a_agg_batch_2), 1).view(len(c_agg_batch), 2, self.embed_d * 2)
        if node_type == 1:
            print(self.a_neigh_att.size())
        ...
        concate_embed = torch.cat((c_agg_batch, a_agg_batch), 1).view(len(c_agg_batch), 2, self.embed_d)
