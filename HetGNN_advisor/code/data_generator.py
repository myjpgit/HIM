...
class input_data(object):
    def __init__(self, args):
        ...
        if self.args.train_test_label != 2:
            ...
            # 			# p_a_e_f = sio.loadmat('../data/academic/node_feature.mat')
            # 			# p_abstract_embed = p_a_e_f['node_feature']
            ...
            net_e_f = sio.loadmat('../data/academic/aut_institution.mat')
            a_net_embed = net_e_f['author_institution']
            a_net_embed = a_net_embed.A
            # p_net_embed = net_e_f['node_feature']
    ...
    def het_walk_restart(self):
        ...
        for i in range(1):
            for j in range(node_n[i]):
                if i == 0:
                    neigh_temp = self.a_a_list_train[j]
                ...
                if len(neigh_temp):
                    ...
                    while neigh_L < 100: #maximum neighbor size = 100
                        ...
                        if rand_p > 0.5:
                            if curNode[0] == "a":
                                curNode = random.choice(self.a_a_list_train[int(curNode[1:])])
        ...
        neigh_f = open(self.args.data_path + "het_neigh.txt", "w")
        for i in range(1):
            for j in range(node_n[i]):
                if i == 0:
                    neigh_train = a_neigh_list_train[j]
